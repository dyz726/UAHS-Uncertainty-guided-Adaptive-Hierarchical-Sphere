...
def AUC_Judd(saliency_map, fixation_map, jitter=True):
    ...
    if s_map.shape != f_map.shape:
        s_map = resize(s_map, f_map.shape, order=1)
                                                                                 
    if jitter:
        s_map = s_map.astype(np.float64) + random.rand(*s_map.shape) * 1e-7
...
shuff_size={
    "360_Saliency_dataset_2018ECCV":(256,128),
    "SVGC_AVA":(360,180),
    "VR-EyeTracking":(800,400),
    "default": (480, 640)
}

def getshuffix(ALLFixPts, size=None, DataSet='SVGC_AVA', nframes=150):
    if size is None:
...
def KLD(saliency_map1, saliency_map2):
    ...
    if map1.shape != map2.shape:
        map1 = resize(map1, map2.shape, order=1)                                                             
...
def EMD(saliency_map1, saliency_map2, sub_sample=1/32.0):
    ...
    map2 = resize(map2, np.round(np.array(map2.shape)*sub_sample), order=1)
